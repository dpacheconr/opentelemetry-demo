...
Scenarios are failure modes you can turn on via feature flags (see [Accessing the Flagd UI](#accessing-the-flagd-ui)) to demonstrate New Relic detecting and diagnosing a problem — for example during a MARS Gameday. See [`newrelic/scenarios`](scenarios/) for the full list and walkthroughs, including [Kafka Consumer Lag](scenarios/kafka-consumer-lag.md).
